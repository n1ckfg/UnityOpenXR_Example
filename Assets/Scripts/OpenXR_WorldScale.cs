...
public class OpenXR_WorldScale : MonoBehaviour {
    ...
    public OpenXR_NewController xrCtlMain;
    public OpenXR_NewController xrCtlAlt;
    ...
    private void Update() {
        if (xrCtlMain.gripDown || xrCtlAlt.gripDown) {
            target.SetParent(origParent);
            armed = true;
        }

        if (xrCtlMain.gripped && xrCtlAlt.gripped) {
            sevenMode = SevenMode.BOTH;
        } else if (xrCtlMain.gripped && !xrCtlAlt.gripped) {
        ...
        if (armed) {
            switch (sevenMode) {
                case SevenMode.BOTH:
                    ...
                    break;
                case SevenMode.MAIN:
                    attachTargetOne(ref xrCtlMain);
                    break;
                case SevenMode.ALT:
                    attachTargetOne(ref xrCtlAlt);
                    break;
            }
            armed = false;
        }
    ...
    private void attachTargetBoth() {
        initialHandPosition1 = xrCtlMain.transform.position;
        initialHandPosition2 = xrCtlAlt.transform.position;
    ...
    private void updateTargetBoth() {
        Vector3 currentHandPosition1 = xrCtlMain.transform.position; // current first hand position
        Vector3 currentHandPosition2 = xrCtlAlt.transform.position; // current second hand position
